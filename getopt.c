...
int rtcsp_getopt(int argc, char* const *argv, const opt_struct opts[], char **optarg, int *optind, int show_err, int arg_start) /* {{{ */
{
	static int optchr = 0;
	...
	if ((argv[*optind][0] == '-') && (argv[*optind][1] == '-')) {
		...
		while (1) {
			rtcsp_optidx++;
			if (opts[rtcsp_optidx].opt_char == '-') {
				(*optind)++;
				return(opt_error(argc, argv, *optind-1, optchr, OPTERRARG, show_err));
			} else if (opts[rtcsp_optidx].opt_name && !strncmp(&argv[*optind][2], opts[rtcsp_optidx].opt_name, arg_end) && arg_end == strlen(opts[rtcsp_optidx].opt_name)) {
				break;
			}
		}
		...
		optchr = 0;
		dash = 0;
		arg_start += strlen(opts[rtcsp_optidx].opt_name);
	} else {
		if (!dash) {
			dash = 1;
			optchr = 1;
	...
	if (rtcsp_optidx < 0) {
		while (1) {
			...
			} else if (argv[*optind][optchr] == opts[rtcsp_optidx].opt_char) {
				break;
			}
		}
	}
	if (opts[rtcsp_optidx].need_param) {
		...
		if (!argv[*optind][arg_start]) {
			...
			if (*optind == argc) {
				...
				if (opts[rtcsp_optidx].need_param == 1) {
					return(opt_error(argc, argv, *optind-1, optchr, OPTERRARG, show_err));
				}
			/* Optional value is not supported with -<arg> <val> style */
			} else if (opts[rtcsp_optidx].need_param == 1) {
				*optarg = argv[(*optind)++];
 			}
		...
		return opts[rtcsp_optidx].opt_char;
	} else {
